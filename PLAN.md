# Tessera — modular physical-control system

**Working name.** A *tessera* is the individual tile of a mosaic: the smallest
unit that is already complete, and that composes without a plan for the whole.
Name is an open question (§9); everything else here holds regardless.
...
```jsonc
{
  "src": "tessera/kitchen/north",   // stable identity == topic path
...
- Symbol and footprint libraries: KiCad official libraries, vendored or pinned,
  plus a project-local `tessera.pretty` and `tessera.kicad_sym` for anything
...
```
parts/tessera-core/tessera-core.scad         # PCB carrier, light pipe, cap seat
parts/tessera-cap/tessera-cap.scad           # the pressed surface
parts/tessera-mount-desk/…                   # weighted desk puck (M1 mount)
parts/tessera-mount-plate/…                  # 1-gang plate carrier
parts/tessera-mount-din/…                    # DIN clip
apothecary/projects/parts/tessera_core.py    # BasePart wrapper + Params
```
...
branch `project/tessera` on the qm repo carrying this project's `adr/`,
`adr-lint.yml` copied unmodified, `project-seed/ide/` copied recursively with
symlinks preserved, license gate wired.

```
tessera/
├── governance/qm/              # submodule, branch project/tessera
...
5. `apothecary parts info tessera-core` returns the part with non-null bounds,
...
1. **Name.** "Tessera" is a placeholder with a defensible etymology and no
   conflict found in the smart-home space. Worth weighing before any repository
   exists.
